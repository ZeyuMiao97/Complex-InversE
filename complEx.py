...
class ComplEx(TensorFactorizer):
	...
	def decompress(self):
		self.rel_emb_real = tf.layers.dense(self.rel_emb_real, 400)
		self.rel_emb_img = tf.layers.dense(self.rel_emb_img, 400)
		self.ent_emb_real = tf.layers.dense(self.ent_emb_real, 400)
		self.ent_emb_img = tf.layers.dense(self.ent_emb_img, 400)
	...
	def define_regularization(self):
		...
		# self.regularizer = (tf.nn.l2_loss(self.rel_emb_real_decompress) + tf.nn.l2_loss(self.rel_emb_img) + tf.nn.l2_loss(self.ent_emb_real) + tf.nn.l2_loss(self.ent_emb_img)) / self.num_batch
	...
	def gather_train_embeddings(self):
		...
		# self.rel_real  = tf.gather(self.rel_emb_real_decompress, self.rel)
