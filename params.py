...
class Params:
	...
	def set_values(self, lr=-1, gamma=-1, bsize=-1, max_itr=-1, p_norm=-1, emb_size=-1, alpha=-1, neg_ratio=-1, save_after=-1, save_each=-1):
		...
		self.gamma = gamma #the margin used in margin-based loss functions基于magin的损失函数中使用的magin
		self.batch_size = bsize
		self.max_iterate = max_itr
		self.p_norm = p_norm #the p_norm (1 or 2) used in margin-based loss functions基于边距的损失函数中使用的p_norm（1或2）
		self.emb_size = emb_size
		self.alpha = alpha #the regularization parameter正则化参数
		self.neg_ratio = neg_ratio
		self.save_each = save_each
		self.save_after = save_after

	def use_default(self, dataset, model):
		if dataset == "wn18" and model == "SimplE_avg":
		...
		elif dataset == "wn18" and model == "ED_SimplE":
			# self.set_values(lr=0.1, alpha=0.03, bsize=1415, max_itr=1000, emb_size=200, neg_ratio=1, save_after=100, save_each=100)
			self.set_values(lr=0.15, alpha=0.03, bsize=1415, max_itr=1500, emb_size=200, neg_ratio=1, save_after=100, save_each=100)


		elif dataset == "fb15k" and model == "SimplE_avg":
			self.set_values(lr=0.05, alpha=0.1, bsize=4832, max_itr=500, emb_size=200, neg_ratio=10, save_after=100, save_each=100)
		elif dataset == "fb15k" and model == "ED_SimplE":
			self.set_values(lr=0.05, alpha=0.1, bsize=4832, max_itr=100, emb_size=200, neg_ratio=10, save_after=100, save_each=100)
			...
			# self.set_values(lr=0.05, alpha=0.01, bsize=4832, max_itr=100, emb_size=200, neg_ratio=10, save_after=100, save_each=100)
